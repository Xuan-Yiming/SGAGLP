...
//						    scaled_map;	  // Scaled image according to WINDOW_WIDTH
	private int[][] original_map, // The image that we will check the obstacles (In case that like someone draw something to image, edited_map is not safe)
					edited_map,	  // The image that was drew on
					scaled_map;	  // Scaled image according to WINDOW_WIDTH
	 ...
	 */
    public World(List<Point> obstacles, int width, int height) {
//    	this.mapFile = mapFile;
		original_map = new int[height][width];
		edited_map = new int[height][width];
		scaled_map = new int[height][width];
		...
		for (Point obstacle : obstacles) {
			original_map[obstacle.y][obstacle.x] = 1;
			edited_map[obstacle.y][obstacle.x] = 1;
			scaled_map[obstacle.y][obstacle.x] = 1;
...
//    	scaled_map = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
    ...
    public int getWidth() {
    	return edited_map[0].length;
    }
    
    public int getHeight() {
    	return edited_map.length;
    }
...
//    }
    ...
    public void clearDrawings() {
		edited_map = new int[original_map.length][original_map[0].length];
		for (Point obstacle : obstacles) {
			edited_map[obstacle.y][obstacle.x] = 1;
		}
    }
...
//    }
    ...
    private boolean isInside(int[][] bi, int x, int y) {
    	int w,h;
    	w = bi[0].length;
    	h = bi.length;
    	if (0<=x && 0<=y && x<w && y<h) {
