...
public class Map {
	...
	private Node[][] edited_map;
	private Node[][] original_map;
...
//    	this.mapFile = mapFile;
    	edited_map = map; //loadPNG(mapFile);
...
//    	}
		...
		// if the node is a block, it is an obstacle
		if (edited_map[y][x] instanceof Block) {
			return true;
		}
    	return false;
    }
...
//    }
    ...
    private boolean isInside(Node[][] bi, int x, int y) {
    	int w,h;
    	w = bi[0].length;
    	h = bi.length;
    	if (0<=x && 0<=y && x<w && y<h) {
