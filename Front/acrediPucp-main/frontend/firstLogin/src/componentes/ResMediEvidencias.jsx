...
import {addBanderaCargarMedicion,addBanderaCargarCalificacion,addBanderaCargarEvidenciaMuestra,addBanderaCargarEvidenciaCompetencia} from "../Redux/CargandoSlice"


export default function ResMediEvidencias({
    cambiarComponente, cambiarComponente2, cambiarComponente3, cambiarComponente4, cambiaId, cambiaIndicador }) {
    ...
    useEffect(()=>{
            ...
            // setBandera(true);
            dispatch(addBanderaCargarEvidenciaMuestra(true));
    ...
    const abreModal = (idComp, iCod, iIndicador) => {
    ...
    const abreDetalle = (idCom,idIndi) => {
        dispatch(addBanderaCargarEvidenciaCompetencia(false));
        setTimeout(async () => {
        cambiarComponente(false)
        cambiarComponente2(false)
        cambiarComponente3(false)
        cambiarComponente4(true)
        cambiaId(idCom)
        cambiaIndicador(idIndi)
    },800);
    }
            ...
            <div className='RMEContenedorSecundario'>
                        ...
                        <table className="tablaF">
                            <tbody>
                                <tr className="tablaEncabezadoMM, eviEncabezado">
                                    <td className="eviEncabezado compAsig"> Competencias Asignadas</td>
                                    <td className="eviEncabezado indicCon">Indicadores</td>
                                    <td className="eviEncabezado evidSol">Evidencia solicitada</td>
                                    ...
                                    datosMedicion.competencias.map((i, index) => {
                                        ...
                                        return (
                                            ...
                                            <>
                                                    {indicadores[index].map((ind) => {
                                                        ...
                                                        return (
                                                            <tr className="tablaEncabezadoMM">
                                                                   ...
                                                                   {ind.evidencia===null?
                                                                  <td>No registrado</td>:
                                                                  <td className='evidenciaRowSize'>{ind.evidencia}</td>}
                                                    ...
                                                    Ver </button></td>
                                                {datosEnviado.enviado ? null : <td className='VerArchivos'><button className="btnRMEGuardar cargar2" type="button"
                                                    data-bs-toggle="modal" data-bs-target="#staticBackdrop" onClick={() => { abreModal(i.idCompetencia, ind.codigoIndicador, ind.fidIndicador) }}>
                                                    Cargar </button></td>}
                                                            </tr>
                                                        )
                ...
                <div className='RMEContenedor2'>
                    ...
                    <div className='RMEContenedorBotones'>
                        ...
                        {openModal && <ModalDrag closeModal={setOpenModal} idCompe={idC} idCod={idCodigo} idInd={idI}/>}
