...
function ModalArchivosActividades({ validar, close, id }) {
    ...
    const subir = async () => {
        ...
        await axios.post("http://localhost:3050/uploadEvidenciaActividad", f, config)
            .then(response => {
                console.log(response.data);
        ...
        <div className='evidencias' >
            {valido ? <div>
                ...
                <div className='cuadroArchivo' >
                    ...
                    <p style={{display:"inline", fontSize:16}}>Máximo tamaño de archivos por subida: 512 MB</p>
                    
                    <p style={{display:"inline", marginLeft:"28%", color:textoColor, fontSize:16}}>Tamaño actual: {totalSize} MB</p>
                    {valido3 && <p style={{  marginLeft:"77%",color:textoColor, fontSize:16}}> (Capacidad excedida)</p>}

                    </div>

                        {
                            fileList.length > 0 ? (
                                <p className="drop-file-preview__title" style={{fontSize:16}}>
                                    Archivos por enviar:
                                </p>
                            ...
                            fileList.map((item, index) => (
                                <div key={index} className="drop-file-preview__item">
                                    ...
                                    <div className="drop-file-preview__item__info">
                                        <p style={{fontSize:16}} className="tituloArchivo" onClick={() => descargar(item)}>{item.name}</p>
                                        <p style={{fontSize:16}}>{(item.size/1000).toFixed(2)} KB</p>
                                    </div>
                    ...
                    {
                        fileList.length > 0 ? (
                            <button disabled={valido3} style={{ borderWidth: '0px', backgroundColor: colorBoton, fontSize:16 }} className="boton-evidencias" onClick={subir}>Subir evidencias</button>


                        ) : (
                            <button disabled={true} style={{ borderWidth: '0px', backgroundColor: "#ADADAD", fontSize:16 }} className="boton-evidencias" onClick={subir}>Subir evidencias</button>
...
export default ModalArchivosActividades;