...
import { addBanderaCargandoEvidencia,addBanderaEliminaEvidenciaActividad } from '../Redux/CargandoSlice';

export default function ResEspeDetalleArchivosActividades(props) {
    const archivos = [
 ...
 */
    ...
    const eliminarArchivosSeleccionados = async () => {
        dispatch(addBanderaEliminaEvidenciaActividad(false));
        ...
        filasMarcadas.map((archivo, index) => {
            const datos = {
                idDetalleEvidenciaActividad: archivo.idDetalleEvidenciaActividad,
                fidActividad: archivo.fidActividad,
        ...
        console.log(data)
        await axios.post("http://localhost:3050/api/actividad/eliminarEvidencia", data, config)
            .then(response => {
                console.log(response.data);
                //setLista(response.data.data)

                setMuestraModal(!response.data.success)
                llamaListaArchivos();
                dispatch(addBanderaEliminaEvidenciaActividad(true));
            }).catch(error => {
                console.log(error);
            })
    ...
    const descargarArchivo = async (tokenArchivo) => {
        ...
        console.log(data)
        await axios.post("http://localhost:3050/api/actividad/descargarEvidencia", data, config)
            .then(response => {
                console.log(response.data);
    ...
    const llamaListaArchivos = async () => {
        ...
        console.log("LISTAR")
        await axios.post("http://localhost:3050/api/actividad/listarEvidencia", data, config)
            .then(response => {
                console.log(response.data);
                setLista(response.data.data)
        ...
        <div className="contedorGeneralRMDA">
            ...
            {muestraModal &&
                <Modal show={muestraModal} onHide={handleModalClose}>
                    <Modal.Body >
                        <p>¿Está seguro que desea eliminar los archivos seleccionados?</p>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button className="botonCancelarAC" onClick={handleModalClose}>
                            Cancelar</Button>
                        <Button className="botonAceptarAC" onClick={eliminarArchivosSeleccionados}>
                            Aceptar</Button>
                    </Modal.Footer>
                </Modal>
            }
            ...
            <div className="contenedorRMDA">
                ...
                <div className="parteSubRMDA" >
                 ...
                 <button disabled={deshabilitado} type="button" class="btnDisenio btnEliminarRMDA" style={{ borderWidth: '0px', backgroundColor: botonColor }} onClick={elimina}>Eliminar</button>
                       
                 {valido2&&<p className="labelGen, labelVDMM" style={{color:"#042354", textAlign:"center"}}>No se encuentran evidencias asociadas a esta actividad</p>}
                ...
                <div className="parteArchivos">
                    ...
                    {lista.map((archivo, index) => (
                        ...
                        <div className="listaArchivosRMDA" style={{width:"22%"}}>
                            ...
                            <FontAwesomeIcon className={obtenerColor(archivo.rutaEvidenciaActividad)} icon={obtenerIcono(archivo.rutaEvidenciaActividad)} />
                          
                            <div className="nombreArchivo" onClick={() => { descargarArchivo(archivo.tokenFoto) }} >
                           
                        <BootstrapTooltip title={archivo.rutaEvidenciaActividad} arrow>
                            {archivo.rutaEvidenciaActividad}
