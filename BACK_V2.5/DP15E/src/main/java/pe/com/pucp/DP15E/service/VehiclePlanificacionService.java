...
import pe.com.pucp.DP15E.repository.VehiclePlanificacionRepository;
...
@Component
public class VehiclePlanificacionService {
    private final VehiclePlanificacionRepository vehiclePlanificacionRepository;
    @Autowired
    public VehiclePlanificacionService(VehiclePlanificacionRepository vehiclePlanificacionRepository) {
        this.vehiclePlanificacionRepository = vehiclePlanificacionRepository;
    }

    public List<VehiclePlanificacion> listarDataImportanteVehiculo(){
        List<Object[]> resultList = vehiclePlanificacionRepository.listarDataImportanteVehiculo();
        List<VehiclePlanificacion> vehicles = new ArrayList<>();

        for (Object[] result : resultList) {
            VehiclePlanificacion vehicle = new VehiclePlanificacion();
    ...
    public  String cargaMasivaDeFlotas(MultipartFile file){
        ...
        List<VehiclePlanificacion> lista = vehiclePlanificacionRepository.findAll();
        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream()))){
            ...
            String line;
            List<VehiclePlanificacion> ListaVehicle = new ArrayList<>();
            //br.readLine();
    ...
    public int buscarRepetidos(VehiclePlanificacion abuscar, List<VehiclePlanificacion> lista){
        if((lista.size())==0) return 0;
        for (VehiclePlanificacion cal: lista) {
            if(
    ...
    private void insertVehicleSimulacion(Connection connection, VehiclePlanificacion vehicle) throws SQLException {
        String sql = "INSERT INTO vehiculo_planificacion (coordenadaX, coordenadaY, peso_bruto, carga_GLP,peso_neto,carga_petroleo,velocidad,mantenimiento," +
